...
def load_data(path: Path) -> pd.DataFrame:
    for params in ({}, {"sep": ";", "encoding": "cp1251"}, {"sep": ";"}, {"encoding": "cp1251"}):
        try:
...
def filter_data(df: pd.DataFrame) -> pd.DataFrame:
    df = df[(df["year_build"] >= 1964) & (df["year_build"] <= 1972)]
    df = df[df["period_start_date"] == "01.07.2025"]
    return df
...
def build_map(input_file: Path, geo_path: Path, output_file: Path) -> None:
    ...
    df = filter_data(df)
    if df.empty:
        raise SystemExit("После фильтрации данных не осталось.")
    ...
    colormap = linear.Greys_09.scale(float(values.min()), float(values.max()))
    colormap.caption = "Медиана цены за кв.м (1964-1972, сделки 01.07.2025)"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Карта медиан цен за кв.м (1964-1972, сделки 01.07.2025)")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=MAPS_DIR / "brick_deal_median_bw_1964_1972.html",
